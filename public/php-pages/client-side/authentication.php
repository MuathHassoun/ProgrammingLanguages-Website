...
#[NoReturn] function afterSuccess(): void {
  if (isset($_SESSION['send_from'])) {
    if ($_SESSION['send_from'] == "edit-languages-page") {
      ...
      header("Location: edit_languages.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['login'])) {
    ...
    if (str_contains($username, "admin")) {
      $stmt = $conn->prepare("SELECT username, full_name, password FROM admins WHERE username = ? LIMIT 1");
    } else {
      $stmt = $conn->prepare("SELECT username, full_name, password FROM users WHERE username = ? LIMIT 1");
    }
    ...
    if ($user && password_verify($password, $user['password'])) {
      ...
      $_SESSION['returned-full-name'] = $user['full_name'];
      if (str_contains($username, "admin")) {
    ...
    if ($username && $email && $password) {
      if (strlen($username) < 10 || strlen($username) > 15) {
        ...
        if (str_contains($username, "admin")) {
          ...
          $stmt = $conn->prepare("INSERT INTO admins (username, full_name, admin_photo, email, password,
                  short_self_introduction, 	long_self_introduction) VALUES (?, ?, ?, ?, ?, ?, ?)");
          $stmt->bind_param("sssssss", $username, $fullName, $photo, $email, $hashedPassword, $shortIntro, $longIntro);
          ...
          $stmt->bind_param("sss", $username, $fullName, $email, $hashedPassword);
        }
        ...
        try {
          ...
          if ($stmt->execute()) {
            ...
            $_SESSION['returned-full-name'] = $fullName;
            if (str_contains($username, "admin")) {
              $_SESSION['isAdmin'] = true;
            } else {
              $_SESSION['isAdmin'] = false;
    ...
    <div class="container">
      <div class="form-box login fade-in">
        <form action="#" method="post">
          ...
          <div class="forgot-link">
            <a href="forgot_password.php#forgot-password-section">Forgot Password?</a>
          </div>
      ...
      <div class="form-box register fade-in">
        <form action="#" method="post">
          <h1>Registration</h1>
          <div class="input-box">
            <input type="text" name="full-name" placeholder="User full name (10-25 characters)" minlength="10" maxlength="25"  required>
